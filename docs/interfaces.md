...
서버는 기본 `http://127.0.0.1:4318`에서 실행한다. 자동화 클라이언트는 OS 데이터 디렉터리의 `runtime.json`을 **프로그램 안에서** 읽고 `Authorization: Bearer <token>`을 전송한다. 파일 내용이나 토큰을 로그·채팅·CI artifact에 출력하지 않는다. UI는 동일 Origin의 세션 초기화 후 HttpOnly 쿠키를 사용한다.
